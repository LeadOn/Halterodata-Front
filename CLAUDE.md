...
Ce fichier doit rester synchronisé avec `.github/copilot-instructions.md` et `CURSOR.md` (contenu identique, adapté au format de chaque outil). **À chaque modification importante du projet (structure de dossiers, modèle de données, conventions de code, etc.), répercute le changement dans les trois fichiers avant de considérer la tâche terminée.**
